...
```typescript
// 简单的适配器函数
function adaptReviewWordToTypingWord(reviewWord: IWordReviewRecord): WordWithIndex {
  return {
...
// 批量适配
function adaptReviewWordsToTypingWords(reviewWords: IWordReviewRecord[]): WordWithIndex[] {
...
export default function ReviewPractice() {
  ...
  // 创建复习专用的 dispatch
  const reviewDispatch = useCallback((action: TypingStateAction) => {
    // 拦截关键 actions 处理复习逻辑
    switch (action.type) {
...
1. **删除不必要的文件**：
   - ❌ `src/components/WordTyping/index.tsx` - 独立的WordTyping组件
...
2. **实现复习专用组件架构**：
   - ✅ `src/pages/review/practice/components/WordPanel/index.tsx` - 复习专用WordPanel
   - ✅ `src/pages/review/practice/components/WordPanel/components/Word/index.tsx` - 复习专用Word组件
   - ✅ `src/pages/review/practice/components/Progress/index.tsx` - 复习专用Progress组件
