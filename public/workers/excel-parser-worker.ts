 ...
 * 检查内存使用情况
 */
function checkMemoryUsage(): { used: number; limit: number; shouldCleanup: boolean } {
  // 在Worker中，我们无法直接访问performance.memory，使用估算
  const estimatedUsed = currentProcessingData ? JSON.stringify(currentProcessingData).length : 0;
 ...
 * 发送进度消息到主线程
 */
function sendProgress(completedRows: number, totalRows: number, done: boolean = false, memoryInfo?: any) {
  const response: WorkerResponse = {
    ...
    data: {
      ...
      percentage: totalRows > 0 ? Math.round((completedRows / totalRows) * 100) : 0,
      memoryInfo
    }
  };
...
async function validateAndConvertData(
  ...
  for (let i = 0; i < Math.min(5, data.length); i++) {
    const row = data[i];
    // 获取可能的单词字段值
    const wordValue = row.word || row.Word || row['单词'] || row['单词【必填】'] || row['A'] || row[Object.keys(row)[0]];
    ...
    if (wordValue && typeof wordValue === 'string' &&
        ...
        wordValue.length > 0 &&
        /^[a-zA-Z]/.test(wordValue)) { // 确保是以字母开头的单词
      startIndex = i;
      break;
    }
  }
  ...
  const hasWord = 'word' in firstRow || 'Word' in firstRow || '单词' in firstRow || '单词【必填】' in firstRow;
  ...
  // 分块处理数据
  for (let chunkStart = 0; chunkStart < actualData.length; chunkStart += chunkSize) {
    // 检查是否需要取消
    if (shouldCancel) {
      break;
    }
    ...
    for (let i = 0; i < chunk.length; i++) {
      ...
      try {
        ...
        // 获取单词名称 - 支持多种字段名和列位置
        const name = row.word || row.Word || row['单词'] || row['单词【必填】'] || row['A'] || row[Object.keys(row)[0]];
      ...
      // 获取音标 - 支持多种字段名和列位置
      const usphone = row.usphone || row.phonetic || row.Phonetic || row['音标'] || row['美式音标'] || row['美式音标【选填】'] || row['B'] || '';
      const ukphone = row.ukphone || row['英式音标'] || row['英式音标【选填】'] || row['C'] || '';
      ...
      translationMappings.forEach(mapping => {
        ...
        const chinese = mapping.chinese.map(key => row[key]).find(val => val) || '';
        const english = mapping.english.map(key => row[key]).find(val => val) || '';
      ...
      if (oldTranslation && typeof oldTranslation === 'string' && oldTranslation.trim() !== '' && detailed_translations.length === 0) {
        const translations = oldTranslation.split(/[,;，；]/).map(t => t.trim()).filter(Boolean);
        detailed_translations.push(...translations.map(trans => ({
          pos: '',
          chinese: trans,
          english: ''
        })));
      ...
      // 构建用户数据
      const hasUserData = detailed_translations.length > 0 ||
                         sentences.length > 0 ||
      ...
      const wordObj: Partial<ICustomWord> = {
        ...
        errors.push(`第${startIndex + index + 2}行: 数据格式错误 - ${error instanceof Error ? error.message : '未知错误'}`);
      }
    }
...
async function parseExcelInWorker(
  ...
  try {
    // 检测是否为大文件
    const isLargeFile = fileSize > LARGE_FILE_THRESHOLD || enableLargeFileOptimization;

    if (isLargeFile) {
      console.log(`检测到大文件 (${(fileSize / 1024 / 1024).toFixed(2)}MB)，启用优化策略`);
    }
    ...
    // 验证和转换数据（支持大文件优化）
    const { words, errors } = await validateAndConvertData(jsonData, isLargeFile);
