...
function analyzeBundle() {
  ...
  const totalSize = [...jsFiles, ...cssFiles, ...otherFiles].reduce((sum, file) => sum + file.size, 0);
  log(`\n📊 总 Bundle 大小: ${colors.bold}${formatBytes(totalSize)}${colors.reset}`, 'green');
  
  // 分析最大的文件
  const allFiles = [...jsFiles, ...cssFiles, ...otherFiles].sort((a, b) => b.size - a.size);
