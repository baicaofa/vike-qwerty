...
export const DonateCard = () => {
  ...
  const HighlightedText = ({ children, className }: { children: React.ReactNode; className?: string }) => {
    return <span className={`font-bold  ${className ? className : 'text-blue-500'}`}>{children}</span>
  }
                ...
                <div className="flex w-full flex-col justify-center gap-4 bg-white px-2 pb-4 pt-5 dark:bg-gray-800 dark:text-gray-300">
                  ...
                  <div className="flex w-full flex-col gap-4 px-4">
                    <p className="mx-auto px-4 indent-4">
                      您刚刚完成了<HighlightedText> {chapterNumber} </HighlightedText>章节的练习，Keybr 已经陪你走过
                    ...
                    <p className="mx-auto px-4 indent-4">
                      Keybr 已经坚持 <span className="font-medium ">开放源码、无广告、无商业化</span> 运营
                      <HighlightedText className="text-blue-500"> {dayFromQwerty} </HighlightedText> 天，
