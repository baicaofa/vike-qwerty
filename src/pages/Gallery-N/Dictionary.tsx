...
function DictionaryComponent({ dictionary, onClick }: Props) {
  return (
    <div
      className="flex h-40 w-80 items-center justify-center"
      role="button"
      onClick={onClick}
      title="选择词典"
    >
      ...
      <div className="flex h-full w-7/12 flex-col items-start justify-start px-6 pt-2">
        ...
        <div className="mb-0 flex w-full items-center">
          <Progress.Root
            value={10}
            max={100}
            className="mr-4 h-3.5 w-full rounded-full border-2 border-indigo-400 bg-white"
          >
            <Progress.Indicator
              className="h-full -translate-x-px rounded-full bg-blue-400 pl-0"
